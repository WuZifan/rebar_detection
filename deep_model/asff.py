...
class ASFF(nn.Module):
    def __init__(self, level, rfb=False, vis=False):
        ...
        if level == 0:
            # stride = 2 降采样了
            self.stride_level_1 = add_conv(18, self.inter_dim, 3, 2)
            self.stride_level_2 = add_conv(18, self.inter_dim, 3, 2)
            # self.expand = add_conv(self.inter_dim, 1024, 3, 1)
            self.expand = add_conv(self.inter_dim, self.inter_dim, 3, 1)
        elif level == 1:
            # self.compress_level_0 = add_conv(512, self.inter_dim, 1, 1)
            self.compress_level_0 = add_conv(18, self.inter_dim, 1, 1)
            self.stride_level_2 = add_conv(18, self.inter_dim, 3, 2)
...
if __name__ == '__main__':
    a0 = ASFF(level=0)
    # print(a0)
    input0 = Variable(torch.randn(1,18,13,13))
    input1 = Variable(torch.randn(1,18,26,26))
    input2 = Variable(torch.randn(1,18,52,52))
    print(a0(input0,input1,input2).shape)
